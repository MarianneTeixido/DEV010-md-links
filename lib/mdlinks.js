...
function mdlinks(path, validate = false) {
    pathAbs = pathModule.resolve(path);
    return new Promise((resolve, reject) => {
        fs.stat(pathAbs, (error, stats) => {
            if (error) {
                console.log(error)
                ...
                const markdownList = [];
                //console.log(stats)
                if (stats.isFile() && isMarkdownFile(pathAbs)) {
                    //Agrega el path del archivo md al arreglo vacio mdList
                    markdownList.push(pathAbs)
                }
                if (stats.isDirectory()) {
                    ...
                    const markdownPaths = markdownFiles.map((file) => {
                        return pathModule.join(pathAbs, file)
                    });
